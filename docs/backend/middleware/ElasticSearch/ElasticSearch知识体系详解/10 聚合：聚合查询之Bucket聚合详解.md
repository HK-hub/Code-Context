...
tags:
  - ElasticSearch
description: 我们在SQL结果中常有： SELECT COUNT(color) FROM table GROUP BY color ElasticSearch中**桶**在概念上类似于 SQL 的分组（`GROUP BY`），而**指标**则类似于 `COUNT()` 、 `SUM()` 、 `MAX()` 等统...
author: HK意境
order: 10
---
...
![img](assets/es-agg-bucket-10.png)
...
![img](assets/es-agg-bucket-33.png)
