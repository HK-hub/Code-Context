...
tags:
  - Java
  - 并发
description: 本课时主要讲解原子类是如何利用 CAS 保证线程安全的。 要想回答这个问题，首先我们需要知道什么是原子类，以及它有什么作用。 在编程领域里，原子性意味着“一组操作要么全都操作成功，要么全都失败，不能只操作成功其中的一部分”。而 java.util.concurrent.atomic 下的类，就是具有...
author: HK意境
order: 40
---
