...
tags:
  - Java
  - 并发
description: 在上一章节，我们详细学习了`synchronized`的具体使用，深入了解了类锁和实例锁的应用场景、优劣势以及 JDK 1.6 之后的优化。在本节课中，我们将进一步拓展我们的多线程知识，转向学习 Java 中的`Lock`接口。 `Lock`接口提供了一种更加灵活和强大的锁定机制，相较于`synch...
author: HK意境
order: 4
---
...
在 ReentrantLock 中，存在两种可中断锁的 API，`tryLock(long timeout, TimeUnit unit)` 和`lockInterruptibly()` 。tryLock 的方式在上面已经给过案例了，这里只演示`lockInterruptibly`：
