---
title: 06 ThreadLocal 之珍宝：线程的隐秘宝库
...
tags:
  - Java
  - 并发
description: 在之前的章节中，我们已经介绍了线程的并发安全问题，这些问题往往由于多个线程竞争访问同一个临界资源而引发。 为了确保临界区的数据正确性，我们主要学习了两种方式来解决并发安全问题：一种是使用`final`关键字，使数据只允许读取不允许修改；另一种是使用`锁机制`，以确保在同一时刻只有一个线程可以修改临界...
author: HK意境
order: 6
---
