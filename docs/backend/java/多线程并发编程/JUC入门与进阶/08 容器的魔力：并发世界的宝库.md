...
tags:
  - Java
  - 并发
description: 我们在前文学习了如何保证对单个对象并发操作的并发安全性，本章节我们将学习如何对一个容器类内的元素并发操作，保证并发安全性。 首先我们需要知道传统的容器是不支持多线程操作的，譬如 **ArrayList、LinkedList、HashSet、TreeSet、HashMap、TreeMap 等**，它们...
author: HK意境
order: 8
---
