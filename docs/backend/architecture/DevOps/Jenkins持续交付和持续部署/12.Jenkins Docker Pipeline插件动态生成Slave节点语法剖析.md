---
title: 12.Jenkins Docker Pipeline插件动态生成Slave节点语法剖析
...
tags:
  - Jenkins
description: 越来越多的公司和用户在实际工作中引入Jenkins，在使用Jenkins自动化构建部署的过程中，使用动态容器作为构建部署工作的执行环境的方案也越来越多的被采纳，不仅提高了资源利用率，同时还降低配置成本和维护管理成本。 在pipeline语法章节介绍Jenkins agent代理的时候有提到可以使用D...
author: HK意境
order: 12
---
