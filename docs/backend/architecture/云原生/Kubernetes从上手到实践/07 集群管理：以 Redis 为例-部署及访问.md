---
title: 07 集群管理：以 Redis 为例-部署及访问
...
tags:
  - Kubernetes
description: 上节我们已经学习了 `kubectl` 的基础使用，本节我们使用 `kubectl` 在 K8S 中进行部署。 **前面我们已经说过，Pod 是 K8S 中最小的调度单元，所以我们无法直接在 K8S 中运行一个 container 但是我们可以运行一个 Pod 而这个 Pod 中只包含一个 cont...
author: HK意境
order: 7
---
