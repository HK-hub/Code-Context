...
tags:
  - 300分钟吃透分布式缓存-完
description: 本课时我们主要学习 Redis 复制原理，以及复制分析等内容。 为了避免单点故障，数据存储需要进行多副本构建。同时由于 Redis 的核心操作是单线程模型的，单个 Redis 实例能处理的请求 TPS 有限。因此 Redis 自面世起，基本就提供了复制功能，而且对复制策略不断进行优化。 ![img]...
author: HK意境
order: 28
---
