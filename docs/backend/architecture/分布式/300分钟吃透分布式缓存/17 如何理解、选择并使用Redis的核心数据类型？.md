---
title: 17 如何理解、选择并使用Redis的核心数据类型？
...
tags:
  - 300分钟吃透分布式缓存-完
description: 你好，我是你的缓存课老师陈波，欢迎进入第 17 课时“Redis 数据类型”的学习。 Redis 数据类型 首先，来看一下 Redis 的核心数据类型。Redis 有 8 种核心数据类型，分别是 ： - string 字符串类型； - list 列表类型； - set 集合类型； - sorted ...
author: HK意境
order: 18
---
